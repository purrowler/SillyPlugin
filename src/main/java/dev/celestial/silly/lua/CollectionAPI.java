...
// Lifted out of Goofyplugin,
// with permission from both Solina
// and the original author, 4P5.
// Goofyplugin is licensed under
// the Apache 2.0 License.
//
// modified to be (theoretically) more performant
// (by making all the functions static), and fixed
// documentation.
@LuaWhitelist
@LuaTypeDoc(name = "CollectionAPI", value = "collection")
public class CollectionAPI {
    public static CollectionAPI Instance = new CollectionAPI();
    ...
    public static Object sum(@LuaNotNil LuaTable tbl) {
        if (tbl.length() == 0) {
            return LuaValue.NIL;
        }
    ...
    @LuaMethodDoc(
            ...
            value = "collection.difference"
    )
    public static Object difference(@LuaNotNil LuaTable tbl) {
        if (tbl.length() == 0) {
            return LuaValue.NIL;
        }
    ...
    @LuaMethodDoc(
            ...
            value = "collection.product"
    )
    public static Object product(@LuaNotNil LuaTable tbl) {
        if (tbl.length() == 0) {
            return LuaValue.NIL;
        }
    ...
    @LuaMethodDoc(
            ...
            value = "collection.quotient"
    )
    public static Object quotient(@LuaNotNil LuaTable tbl) {
        if (tbl.length() == 0) {
            return LuaValue.NIL;
        }
    ...
    public static LuaTable map(@LuaNotNil LuaTable tbl, LuaFunction func) {
    ...
    public static LuaTable each(@LuaNotNil LuaTable tbl, LuaFunction func) {
        for (LuaValue key : tbl.keys()) {
    ...
    @LuaMethodDoc(
            ...
            value = "collection.flat_map"
    )
    public static LuaTable flatMap(@LuaNotNil LuaTable tbl, LuaFunction func) {
    ...
    public static LuaTable filter(@LuaNotNil LuaTable tbl, LuaFunction func) {
        LuaTable result = new LuaTable();
        for (LuaValue key : tbl.keys()) {
